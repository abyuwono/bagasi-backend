...
router.get('/ad/:adId', auth, async function(req, res) {
  try {
...
// Send message
router.post('/:chatId/messages', auth, async function(req, res) {
  try {
...
router.patch('/:chatId/read', auth, async function(req, res) {
  try {
    const chat = await Chat.findOne({
      _id: req.params.chatId,
      $or: [{ shopper: req.user.id }, { traveler: req.user.id }]
...
router.get('/active', auth, async function(req, res) {
  try {
